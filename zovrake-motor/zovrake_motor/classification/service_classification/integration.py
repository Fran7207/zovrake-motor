"""Integración del SCE con estados y eventos del núcleo."""
...
from zovrake_motor.classification.service_classification.models import ServiceClassificationResult
...
class ServiceClassificationMotorIntegration:
    """
    Puente de integración del SCE con StateManager y EventManager.
    ...
    def begin_service_classification(
        self,
        process_id: UUID,
        *,
        document_id: str,
        model_id: str,
        concept_catalog_id: str,
    ) -> None:
        self._transition_state(
            process_id,
            MotorState.PROCESANDO,
            f"Iniciando clasificación de servicios: {document_id}",
        )
        self._register_event(
            process_id=process_id,
            message=f"Clasificación de servicios iniciada para {document_id}",
            event_type=EventType.MODULE,
            severity=EventSeverity.INFO,
            metadata={
                "document_id": document_id,
                "model_id": model_id,
                "concept_catalog_id": concept_catalog_id,
                "phase": "service_classification",
            },
        )

    def complete_service_classification(
        self,
        process_id: UUID,
        result: ServiceClassificationResult,
    ) -> dict[str, Any]:
        state = self._transition_state(
            process_id,
            MotorState.PROCESAMIENTO_COMPLETADO,
            f"Clasificación de servicios completada: {result.document_id}",
        )
        severity = EventSeverity.WARNING if result.incidents else EventSeverity.INFO
        message = (
            f"Clasificación de servicios con incidencias para {result.document_id}"
            if result.incidents
            else f"Clasificación de servicios completada para {result.document_id}"
        )
        ...
        event = self._register_event(
            ...
            metadata={
                ...
                "services_classified": len(result.catalog.services),
