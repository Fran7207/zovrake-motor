"""Integración del EDE con estados y eventos del núcleo."""
...
from zovrake_motor.classification.equivalence_detection.models import EquivalenceDetectionResult
...
class EquivalenceDetectionMotorIntegration:
    """
    Puente de integración del EDE con StateManager y EventManager.
    ...
    def begin_equivalence_detection(
        self,
        process_id: UUID,
        *,
        document_id: str,
        model_id: str,
        normalized_catalog_id: str,
    ) -> None:
        self._transition_state(
            process_id,
            MotorState.PROCESANDO,
            f"Iniciando detección de equivalencias: {document_id}",
        )
        self._register_event(
            process_id=process_id,
            message=f"Detección de equivalencias iniciada para {document_id}",
            event_type=EventType.MODULE,
            severity=EventSeverity.INFO,
            metadata={
                "document_id": document_id,
                "model_id": model_id,
                "normalized_catalog_id": normalized_catalog_id,
                "phase": "equivalence_detection",
            },
        )

    def complete_equivalence_detection(
        self,
        process_id: UUID,
        result: EquivalenceDetectionResult,
    ) -> dict[str, Any]:
        state = self._transition_state(
            process_id,
            MotorState.PROCESAMIENTO_COMPLETADO,
            f"Detección de equivalencias completada: {result.document_id}",
        )
        severity = EventSeverity.WARNING if result.incidents else EventSeverity.INFO
        message = (
            f"Detección de equivalencias con incidencias para {result.document_id}"
            if result.incidents
            else f"Detección de equivalencias completada para {result.document_id}"
        )
        ...
        event = self._register_event(
            ...
            metadata={
                ...
                "equivalences_detected": len(result.catalog.equivalences),
