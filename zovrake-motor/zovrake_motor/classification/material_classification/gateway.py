"""Gateway de consumo del catálogo de conceptos del CAE — sin acoplamiento externo."""
...
from zovrake_motor.classification.material_classification.exceptions import ConceptCatalogAccessError
...
class ConceptCatalogView:
    ...
    def material_candidate_concepts(self) -> tuple[ConceptCandidate, ...]:
        material_kinds = {ConceptKind.ITEM, ConceptKind.PARTIDA}
        return tuple(concept for concept in self.concepts if concept.kind in material_kinds)


class ConceptCatalogGateway:
    """
    Gateway de consumo del catálogo de conceptos.

    Valida y expone una vista inmutable sin acceder al documento original.
    """
    ...
    def validate(self, catalog_dict: dict[str, Any]) -> ConceptCatalogView:
        ...
        if not catalog.material_classification_prepared:
            raise ConceptCatalogAccessError("El catálogo no está preparado para clasificación de materiales")
