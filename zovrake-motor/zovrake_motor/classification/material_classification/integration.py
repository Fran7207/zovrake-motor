"""Integración del MCE con estados y eventos del núcleo."""
...
from zovrake_motor.classification.material_classification.models import MaterialClassificationResult
...
class MaterialClassificationMotorIntegration:
    """
    Puente de integración del MCE con StateManager y EventManager.
    ...
    def begin_material_classification(
        self,
        process_id: UUID,
        *,
        document_id: str,
        ...
        self._transition_state(
            ...
            f"Iniciando clasificación de materiales: {document_id}",
        )
        self._register_event(
            process_id=process_id,
            message=f"Clasificación de materiales iniciada para {document_id}",
    ...
    def complete_material_classification(
        self,
        process_id: UUID,
        ...
        state = self._transition_state(
            ...
            f"Clasificación de materiales completada: {result.document_id}",
        )
        severity = EventSeverity.WARNING if result.incidents else EventSeverity.INFO
        message = (
            f"Clasificación de materiales con incidencias para {result.document_id}"
            if result.incidents
            else f"Clasificación de materiales completada para {result.document_id}"
        )
        ...
        event = self._register_event(
            ...
            metadata={
                ...
                "materials_classified": len(result.catalog.materials),
