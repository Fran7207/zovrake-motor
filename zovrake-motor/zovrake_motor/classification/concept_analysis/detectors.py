...
from zovrake_motor.classification.concept_analysis.enums import ConceptDetectorType, ConceptKind
...
class TechnicalConceptDetector(ConceptDetectorPort):
    ...
    def detect(self, model_view: InternalModelView, *, start_sequence: int) -> DetectorResult:
        ...
        entity_id = str(technical.get("entity_id", "technical_information"))

        for index, specification in enumerate(technical.get("specifications", ())):
            ...
            concepts.append(
                build_concept(
                    ...
                    entity_id=entity_id,
                    source_reference=str(technical.get("source_reference", "")),
                    canonical_reference=str(technical.get("canonical_reference", "")),
                    extraction_reference=str(technical.get("extraction_reference", "")),
                    entity_index=index,
        ...
        for field_name, value in technical.get("fields", {}).items():
        ...
        return DetectorResult(
            ...
            detector_name=self.detector_name,
            concepts=tuple(concepts),
            technical_observations=("technical_section_scanned=True",),
        )
...
class CommercialConceptDetector(ConceptDetectorPort):
    ...
    def detect(self, model_view: InternalModelView, *, start_sequence: int) -> DetectorResult:
        ...
        entity_id = str(commercial.get("entity_id", "commercial_information"))

        for field_name, label in self.COMMERCIAL_FIELDS:
            description = str(commercial.get(field_name, "")).strip()
            if not description:
                continue
            concepts.append(
                build_concept(
                    ...
                    entity_id=entity_id,
                    source_reference=str(commercial.get("source_reference", "")),
                    canonical_reference=str(commercial.get("canonical_reference", "")),
                    extraction_reference=str(commercial.get("extraction_reference", "")),
                    field_name=field_name,
                    metadata={"commercial_field": field_name},
                ),
            )
            sequence += 1
        ...
        return DetectorResult(
            ...
            detector_name=self.detector_name,
            concepts=tuple(concepts),
            technical_observations=("commercial_section_scanned=True",),
        )
...
class CommercialConditionDetector(ConceptDetectorPort):
    ...
    def detect(self, model_view: InternalModelView, *, start_sequence: int) -> DetectorResult:
        concepts = []
        sequence = start_sequence
        for index, condition in enumerate(model_view.commercial_conditions):
            description = str(condition.get("content", "")).strip()
            if not description:
                continue
            concepts.append(
                build_concept(
                    ...
                    entity_id=str(condition.get("entity_id", condition.get("condition_id", f"cond-{index}"))),
                    source_reference=str(condition.get("source_reference", "")),
                    canonical_reference=str(condition.get("canonical_reference", "")),
                    extraction_reference=str(condition.get("extraction_reference", "")),
                    entity_index=index,
                    metadata={"condition_type": condition.get("condition_type", "")},
                ),
            )
            sequence += 1
        ...
        return DetectorResult(
            ...
            detector_name=self.detector_name,
            concepts=tuple(concepts),
            technical_observations=(f"conditions_scanned={len(model_view.commercial_conditions)}",),
        )


class ObservationConceptDetector(ConceptDetectorPort):
    """Identifica observaciones del modelo interno como conceptos candidatos."""
    ...
    def detect(self, model_view: InternalModelView, *, start_sequence: int) -> DetectorResult:
        concepts = []
        sequence = start_sequence
        for index, observation in enumerate(model_view.observations):
            description = str(observation.get("content", "")).strip()
            if not description:
                continue
            concepts.append(
                build_concept(
                    ...
                    entity_id=str(
                        observation.get("entity_id", observation.get("observation_id", f"obs-{index}")),
                    ),
                    source_reference=str(observation.get("source_reference", "")),
                    canonical_reference=str(observation.get("canonical_reference", "")),
                    extraction_reference=str(observation.get("extraction_reference", "")),
                    entity_index=index,
                    metadata={"observation_type": observation.get("observation_type", "")},
                ),
            )
            sequence += 1
        ...
        return DetectorResult(
            ...
            detector_name=self.detector_name,
            concepts=tuple(concepts),
            technical_observations=(f"observations_scanned={len(model_view.observations)}",),
        )