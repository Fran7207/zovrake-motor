"""Integración del CNE con estados y eventos del núcleo."""
...
from zovrake_motor.classification.concept_normalization.models import ConceptNormalizationResult
...
class ConceptNormalizationMotorIntegration:
    """
    Puente de integración del CNE con StateManager y EventManager.
    ...
    def begin_concept_normalization(
        self,
        process_id: UUID,
        *,
        document_id: str,
        ...
        self._transition_state(
            ...
            f"Iniciando normalización conceptual: {document_id}",
        )
        self._register_event(
            process_id=process_id,
            message=f"Normalización conceptual iniciada para {document_id}",
            ...
            metadata={
                ...
                "phase": "concept_normalization",
            },
        )

    def complete_concept_normalization(
        self,
        process_id: UUID,
        result: ConceptNormalizationResult,
    ) -> dict[str, Any]:
        state = self._transition_state(
            process_id,
            MotorState.PROCESAMIENTO_COMPLETADO,
            f"Normalización conceptual completada: {result.document_id}",
        )
        severity = EventSeverity.WARNING if result.incidents else EventSeverity.INFO
        message = (
            f"Normalización conceptual con incidencias para {result.document_id}"
            if result.incidents
            else f"Normalización conceptual completada para {result.document_id}"
        )
        ...
        event = self._register_event(
            ...
            metadata={
                ...
                "concepts_normalized": len(result.catalog.concepts),
