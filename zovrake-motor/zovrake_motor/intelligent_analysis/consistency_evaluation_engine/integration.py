...
class ConsistencyEvaluationMotorIntegration:
    """
    ...
    def begin_consistency_evaluation(
        self,
        process_id: UUID,
        *,
        document_id: str,
        ...
        self._transition_state(
            ...
            f"Iniciando evaluación de consistencia: {document_id}",
        )
        self._register_event(
            process_id=process_id,
            message=f"Evaluación de consistencia iniciada para {document_id}",
            ...
            metadata={
                ...
                "phase": "consistency_evaluation",
            },
        )

    def complete_consistency_evaluation(
        self,
        process_id: UUID,
        ...
        state = self._transition_state(
            ...
            f"Evaluación de consistencia completada: {result.document_id}",
        )
        severity = EventSeverity.WARNING if result.incidents else EventSeverity.INFO
        message = (
            f"Evaluación de consistencia con incidencias para {result.document_id}"
            if result.incidents
            else f"Evaluación de consistencia completada para {result.document_id}"
        )
