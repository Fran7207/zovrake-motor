"""Integración del RRB con estados y eventos del núcleo."""
...
from zovrake_motor.intelligent_analysis.reasoning_result_builder.models import ReasoningResultBuildResult
...
class ReasoningResultMotorIntegration:
    """Puente de integración del RRB con StateManager y EventManager."""
    ...
    def begin_reasoning_result_build(
        self,
        process_id: UUID,
        *,
        document_id: str,
        model_id: str,
        recommendation_catalog_id: str,
    ) -> None:
        self._transition_state(
            process_id,
            MotorState.PROCESANDO,
            f"Iniciando construcción del Resultado del Análisis Inteligente: {document_id}",
        )
        self._register_event(
            process_id=process_id,
            message=f"Construcción de resultados iniciada para {document_id}",
            ...
            metadata={
                ...
                "phase": "reasoning_result_build",
            },
        )

    def complete_reasoning_result_build(
        self,
        process_id: UUID,
        result: ReasoningResultBuildResult,
    ) -> dict[str, Any]:
        state = self._transition_state(
            process_id,
            MotorState.PROCESAMIENTO_COMPLETADO,
            f"Construcción de resultados completada: {result.document_id}",
        )
        severity = EventSeverity.WARNING if result.incidents else EventSeverity.INFO
        message = (
            f"Construcción de resultados con incidencias para {result.document_id}"
            if result.incidents
            else f"Construcción de resultados completada para {result.document_id}"
        )
