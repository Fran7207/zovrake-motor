"""Integración del RAE con estados y eventos del núcleo."""
...
from zovrake_motor.intelligent_analysis.risk_analysis_engine.models import RiskAnalysisResult
...
class RiskAnalysisMotorIntegration:
    """
    Puente de integración del RAE con StateManager y EventManager.
    ...
    def begin_risk_analysis(
        ...
        *,
        ...
        consistency_catalog_id: str,
    ) -> None:
        self._transition_state(
            process_id,
            MotorState.PROCESANDO,
            f"Iniciando análisis de riesgos: {document_id}",
        )
        self._register_event(
            process_id=process_id,
            message=f"Análisis de riesgos iniciado para {document_id}",
    ...
    def complete_risk_analysis(
        ...
        state = self._transition_state(
            ...
            f"Análisis de riesgos completado: {result.document_id}",
        )
        severity = EventSeverity.WARNING if result.incidents else EventSeverity.INFO
        message = (
            f"Análisis de riesgos con incidencias para {result.document_id}"
            if result.incidents
            else f"Análisis de riesgos completado para {result.document_id}"
        )
