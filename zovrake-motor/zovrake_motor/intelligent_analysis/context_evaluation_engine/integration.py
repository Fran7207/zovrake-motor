"""Integración del CxEE con estados y eventos del núcleo."""
...
from zovrake_motor.intelligent_analysis.context_evaluation_engine.models import ContextEvaluationResult
...
class ContextEvaluationMotorIntegration:
    """
    Puente de integración del CxEE con StateManager y EventManager.
    ...
    def begin_context_evaluation(
        ...
        *,
        ...
        self._transition_state(
            ...
            f"Iniciando evaluación contextual: {document_id}",
        )
        self._register_event(
            process_id=process_id,
            message=f"Evaluación contextual iniciada para {document_id}",
    ...
    def complete_context_evaluation(
        self,
        process_id: UUID,
        ...
        state = self._transition_state(
            ...
            f"Evaluación contextual completada: {result.document_id}",
        )
        severity = EventSeverity.WARNING if result.incidents else EventSeverity.INFO
        message = (
            f"Evaluación contextual con incidencias para {result.document_id}"
            if result.incidents
            else f"Evaluación contextual completada para {result.document_id}"
        )
