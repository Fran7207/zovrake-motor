"""Composición del runtime del Motor para la API pública.

Este módulo actúa como composition root. La capa ``api`` solo expone la
fachada pública y no importa directamente los módulos internos del Motor.
"""
...
class MotorApiRuntime:
    ...
    def snapshot(self) -> dict[str, Any]:
        return {
            ...
            "enterprise_integration_available": (
                self.enterprise_integration.is_available()
            ),
            "modules_registered": (
                self.coordinator.module_administrator.count()
            ),
