"""Integración del POE con estados y eventos del núcleo."""
...
class ProviderOrganizationMotorIntegration:
    """
    Puente de integración del POE con StateManager y EventManager.
    ...
    def begin_provider_organization(
        self,
        process_id: UUID,
        *,
        document_id: str,
        ...
        self._transition_state(
            ...
            f"Iniciando organización de proveedores: {document_id}",
        )
        self._register_event(
            process_id=process_id,
            message=f"Organización de proveedores iniciada para {document_id}",
            ...
            metadata={
                ...
                "phase": "provider_organization",
            },
        )

    def complete_provider_organization(
        self,
        process_id: UUID,
        result: ProviderOrganizationBuildResult,
    ) -> dict[str, Any]:
        state = self._transition_state(
            process_id,
            MotorState.PROCESAMIENTO_COMPLETADO,
            f"Organización de proveedores completada: {result.document_id}",
        )
        total_providers = sum(
        ...
        message = (
            f"Organización de proveedores con incidencias para {result.document_id}"
            if result.incidents
            else f"Organización de proveedores completada para {result.document_id}"
        )
        ...
        event = self._register_event(
            ...
            metadata={
                ...
                "provider_sets_organized": len(result.catalog.provider_sets),
                "providers_organized": total_providers,
