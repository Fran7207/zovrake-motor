"""Gateway de consumo de catálogos del CSE, DCB, DRB y POE."""
...
@dataclass(frozen=True)
class IntegrityValidationInputView:
...
class IntegrityValidationInputGateway:
    """Gateway de consumo de catálogos para el GIE — solo lectura."""
    ...
    def validate(
        ...
        self._validate_consistency(structure_view, column_view, row_view, provider_view)
        return IntegrityValidationInputView(
    ...
    def _validate_provider(self, catalog_dict: dict[str, Any]) -> ProviderCatalogView:
        ...
        if not bool(catalog_dict.get("group_integrity_engine_prepared", True)):
            raise ProviderCatalogAccessError(
                "El catálogo de proveedores no está preparado para validación de integridad",
            )
        ...
        return ProviderCatalogView(
            ...
            source_structure_catalog_id=str(catalog_dict.get("source_structure_catalog_id", "")),
            source_column_catalog_id=str(catalog_dict.get("source_column_catalog_id", "")),
            source_row_catalog_id=str(catalog_dict.get("source_row_catalog_id", "")),
            provider_sets=provider_sets,
            group_integrity_engine_prepared=True,
            domain_model_preserved=bool(catalog_dict.get("domain_model_preserved", True)),
            raw_catalog=catalog_dict,
        )
