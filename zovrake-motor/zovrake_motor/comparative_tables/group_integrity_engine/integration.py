"""Integración del GIE con estados y eventos del núcleo."""
...
class GroupIntegrityMotorIntegration:
    """Puente de integración del GIE con StateManager y EventManager."""
    ...
    def begin_group_integrity_validation(
        self,
        process_id: UUID,
        *,
        document_id: str,
        ...
        self._transition_state(
            ...
            f"Iniciando validación de integridad: {document_id}",
        )
        self._register_event(
            process_id=process_id,
            message=f"Validación de integridad iniciada para {document_id}",
            ...
            metadata={
                ...
                "phase": "group_integrity_validation",
            },
        )

    def complete_group_integrity_validation(
        self,
        process_id: UUID,
        result: GroupIntegrityValidationResult,
    ) -> dict[str, Any]:
        state = self._transition_state(
            process_id,
            MotorState.PROCESAMIENTO_COMPLETADO,
            f"Validación de integridad completada: {result.document_id}",
        )
        severity = (
        ...
        message = (
            f"Validación de integridad con hallazgos para {result.document_id}"
            if result.findings_count
            else f"Validación de integridad completada para {result.document_id}"
        )
