"""Integración del CSE con estados y eventos del núcleo."""
...
class ComparativeStructureMotorIntegration:
    """
    Puente de integración del CSE con StateManager y EventManager.
    ...
    def begin_comparative_structure_build(
        self,
        process_id: UUID,
        *,
        document_id: str,
        ...
        self._transition_state(
            ...
            f"Iniciando construcción de estructuras comparativas: {document_id}",
        )
        self._register_event(
            process_id=process_id,
            message=f"Construcción de estructuras comparativas iniciada para {document_id}",
            event_type=EventType.MODULE,
            severity=EventSeverity.INFO,
            metadata={
                "document_id": document_id,
                "model_id": model_id,
                "domain_catalog_id": domain_catalog_id,
                "phase": "comparative_structure_build",
            },
        )

    def complete_comparative_structure_build(
        self,
        process_id: UUID,
        result: ComparativeStructureBuildResult,
    ) -> dict[str, Any]:
        state = self._transition_state(
            process_id,
            MotorState.PROCESAMIENTO_COMPLETADO,
            f"Construcción de estructuras comparativas completada: {result.document_id}",
        )
        severity = EventSeverity.WARNING if result.incidents else EventSeverity.INFO
        message = (
            f"Construcción de estructuras comparativas con incidencias para {result.document_id}"
            if result.incidents
            else f"Construcción de estructuras comparativas completada para {result.document_id}"
        )
        ...
        event = self._register_event(
            ...
            metadata={
                ...
                "structures_built": len(result.catalog.structures),
