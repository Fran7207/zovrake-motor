"""Integración del DRB con estados y eventos del núcleo."""
...
from zovrake_motor.comparative_tables.dynamic_row_builder.models import ComparativeRowBuildResult
...
class DynamicRowMotorIntegration:
    """
    Puente de integración del DRB con StateManager y EventManager.
    ...
    def begin_dynamic_row_build(
        self,
        process_id: UUID,
        *,
        document_id: str,
        model_id: str,
        column_catalog_id: str,
        structure_catalog_id: str,
    ) -> None:
        self._transition_state(
            process_id,
            MotorState.PROCESANDO,
            f"Iniciando construcción de filas dinámicas: {document_id}",
        )
        self._register_event(
            process_id=process_id,
            message=f"Construcción de filas dinámicas iniciada para {document_id}",
            ...
            metadata={
                ...
                "phase": "dynamic_row_build",
            },
        )

    def complete_dynamic_row_build(
        self,
        process_id: UUID,
        result: ComparativeRowBuildResult,
    ) -> dict[str, Any]:
        state = self._transition_state(
            process_id,
            MotorState.PROCESAMIENTO_COMPLETADO,
            f"Construcción de filas dinámicas completada: {result.document_id}",
        )
        ...
        message = (
            f"Construcción de filas dinámicas con incidencias para {result.document_id}"
            if result.incidents
            else f"Construcción de filas dinámicas completada para {result.document_id}"
        )
        ...
        event = self._register_event(
            ...
            metadata={
                ...
                "row_sets_built": len(result.catalog.row_sets),
                "rows_built": total_rows,
