...
from zovrake_motor.comprehension.knowledge_index.entry_builder import (
    IndexEntryBuilder,
)
from zovrake_motor.comprehension.knowledge_index.gateway import (
...
from zovrake_motor.comprehension.knowledge_index.query_hook import (
    QueryIntegrationPoint,
)
from zovrake_motor.comprehension.knowledge_index.reuse_hook import (
    ReuseIntegrationPoint,
)
from zovrake_motor.comprehension.knowledge_index.store import (
    KnowledgeIndexStore,
)
...
class DocumentKnowledgeIndex:
    ...
    def query_integration(self) -> QueryIntegrationPoint:
        if self._query_hook is None:
            self._query_hook = QueryIntegrationPoint(
    ...
    def reuse_integration(self) -> ReuseIntegrationPoint:
        if self._reuse_hook is None:
            self._reuse_hook = ReuseIntegrationPoint(
                settings=self._index_settings()
            )
    ...
    def register(
        ...
        registered_entry = self._store.register(entry)

        reused_existing_entry = (
            registered_entry is not entry
        )

        entry = registered_entry

        query_status = (
            self.query_integration.prepare_for_future_queries(
                query_keys=entry.query_keys
            )
        )

        reuse_status = (
            self.reuse_integration.prepare_for_future_reuse(
                entry
            )
        )
        ...
        observations = (
            *entry.technical_observations,
            ...
            f"existing_entry_reused={reused_existing_entry}",
    ...
    def snapshot(self) -> dict[str, Any]:
        ...
        return {
            ...
            "configuration": {
                ...
                "query_integration_prepared": (
                    settings.query_integration_prepared
                ),
                "reuse_integration_prepared": (
                    settings.reuse_integration_prepared
                ),
                "max_entries_in_memory": (
                    settings.max_entries_in_memory
                ),
            },
        }