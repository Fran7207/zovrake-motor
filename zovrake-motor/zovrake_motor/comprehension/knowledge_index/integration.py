"""Integración del DKI con estados y eventos del núcleo."""
...
class KnowledgeIndexMotorIntegration:
    """
    Puente de integración del DKI con StateManager y EventManager.
    ...
    def begin_indexing(self, process_id: UUID, *, document_id: str) -> None:
        self._transition_state(
            process_id,
            MotorState.PROCESANDO,
            f"Iniciando indexación documental: {document_id}",
        )
        self._register_event(
            process_id=process_id,
            message=f"Indexación documental iniciada para {document_id}",
            event_type=EventType.MODULE,
            severity=EventSeverity.INFO,
            metadata={"document_id": document_id, "phase": "knowledge_index"},
        )
    ...
    def complete_indexing(
        ...
        state = self._transition_state(
            ...
            f"Indexación documental completada: {result.document_id}",
        )
        severity = EventSeverity.WARNING if result.incidents else EventSeverity.INFO
        message = (
            f"Indexación con incidencias para {result.document_id}"
            if result.incidents
            else f"Indexación exitosa para {result.document_id}"
        )
        ...
        event = self._register_event(
            ...
            metadata={
                ...
                "model_reference": result.entry.model_reference,
