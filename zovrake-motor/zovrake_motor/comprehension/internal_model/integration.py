"""Integración del IDMB con estados y eventos del núcleo."""
...
class InternalModelMotorIntegration:
    """
    Puente de integración del IDMB con StateManager y EventManager.
    ...
    def begin_model_build(self, process_id: UUID, *, document_id: str) -> None:
        self._transition_state(
            process_id,
            MotorState.PROCESANDO,
            f"Iniciando construcción del modelo interno: {document_id}",
        )
        self._register_event(
            process_id=process_id,
            message=f"Construcción del modelo interno iniciada para {document_id}",
            event_type=EventType.MODULE,
            severity=EventSeverity.INFO,
            metadata={"document_id": document_id, "phase": "internal_model_build"},
        )

    def complete_model_build(
        self,
        process_id: UUID,
        result: InternalModelBuildResult,
    ) -> dict[str, Any]:
        state = self._transition_state(
            process_id,
            MotorState.PROCESAMIENTO_COMPLETADO,
            f"Modelo interno construido: {result.document_id}",
        )
        severity = EventSeverity.WARNING if result.incidents else EventSeverity.INFO
        message = (
            f"Modelo interno con incidencias para {result.document_id}"
            if result.incidents
            else f"Modelo interno construido exitosamente para {result.document_id}"
        )
        ...
        event = self._register_event(
            ...
            metadata={
                ...
                "immutable": result.model.immutable,
