"""Prueba de equivalencia colectiva entre documentos reales."""
...
def test_collective_equivalence_detects_cross_document_match() -> None:
    ...
    result = executor._run_collective_equivalence_detection(
        process_id=PROCESS_ID,
        collective_normalized_catalog=collective,
    )

    assert result["equivalences_count"] >= 1

    equivalence = result["equivalences"][0]
    traceability = equivalence["traceability"]

    assert traceability["document_ids"] == ["DOC-A", "DOC-B"]
    assert set(traceability["concept_ids"]) == {"CONCEPT-A", "CONCEPT-B"}


def test_collective_equivalence_does_not_match_same_document_pairs() -> None:
    executor = CotizacionesAnalysisExecutor(
        result_registry=AnalysisResultRegistry(),
    )
    executor.initialize()
    ...
    collective = {
        "catalog_id": f"cne-collective://{PROCESS_ID}/same-document",
        "process_id": str(PROCESS_ID),
        "model_id": f"collective-model://{PROCESS_ID}/same-document",
        "document_id": f"multi-document://{PROCESS_ID}/same-document",
        "document_ids": ["DOC-A"],
        "document_count": 1,
        "source_catalog_ids": ["CAT-A"],
        "source_model_ids": ["MODEL-A"],
        "concepts": [
            _concept("DOC-A", "CONCEPT-A1"),
            _concept("DOC-A", "CONCEPT-A2"),
        ],
        "concepts_count": 2,
        "equivalence_detection_prepared": True,
        "collective_normalization": True,
    ...
    assert result["equivalences_count"] == 0